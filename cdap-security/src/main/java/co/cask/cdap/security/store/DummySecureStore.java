...
public class DummySecureStore implements SecureStore, SecureStoreManager {
  ...
  private static final String SECURE_STORE_SETUP = "Secure store is not configured. To use secure store the provider " +
    ...
    "The value could either be \"kms\" for Hadoop KMS based provider or \"file\" of Java JCEKS based provider. " +
    "Both without quotes. " +
    "KMS based provider is supported for Apache Hadoop 2.6.0 and up and on distribution versions that are based " +
    "on Apache Hadoop 2.6.0 and up. " +
    "Java JCEKS based provider is not supported in distributed mode. If the provider is set to file then the user " +
    "must provide the password to be used to access the keystore. The password can be set using " +
    "\"security.store.file.password\" property in cdap-security.xml. " +
    "For more details please refer to https://wiki.cask.co/display/CE/Using+Secure+Store.";
